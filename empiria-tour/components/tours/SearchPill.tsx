...
/**
 * The segmented search control — Where / When / Who as one pill.
 *
 * Airbnb's shape, because travellers already know it and inventing a new one
 * costs conversions. Three decisions that differ from theirs:
 *
 ...
 *  - No infant field here. Infants do not affect what a search returns, only
 *    what a booking costs, so it belongs on the package page.
 */
export default function SearchPill({
  ...
  const segInput =
    'w-full bg-transparent text-[15px] text-ink outline-none placeholder:text-stone/60';
  ...
  return (
    <form
      ...
      className={`search-pill flex w-full flex-col md:flex-row md:items-stretch ${
        compact ? 'p-1.5' : 'p-2'
      }`}
    >
      <div className="search-seg flex-1 px-4 py-3 md:px-5">
        ...
        <select
          ...
          className={`${segInput} cursor-pointer`}
        >
      ...
      <div className="flex items-center justify-end p-2 md:p-1.5">
        <button
          type="submit"
          className="flex h-12 w-full items-center justify-center gap-2 rounded-full bg-flame px-6 font-mono text-[11px] font-bold uppercase tracking-label text-white transition-colors hover:bg-ember md:w-auto"
        >
